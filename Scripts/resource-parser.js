...
function HPS2QX(subs, Ptfo, Pcert0, PTls13) {
  ...
  if (server != "") {
    ...
    var thost = subs.indexOf("peer=") != -1 ? "tls-host=" + subs.split("peer=")[1].split("#")[0].split("&")[0].replace(/"/g, "") : "" // 存在peers参数时 https://b64(ipport)?peer=xxx#server-remarks
...
function VQ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (server.indexOf("obfs=") == -1) { // 非 ws/http 类型
    ...
    var host = server.indexOf("tls-host") != -1 ? "obfs-host=" + server.split("tls-host=")[1].split(",")[0].replace(/"/g, "") + ", " : ""
    obfs = obfs + host
    ...
    obfs = obfs + uri
    var host = server.indexOf("obfs-header=") != -1 ? "obfs-host=" + server.split("obfs-header=\"Host:")[1].split("[")[0].replace(/"/g, "").trim() + ", " : ""
    obfs = obfs + host
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (obfs == "none") { //
    ...
    host = host != "{}" && host ? "obfs-host=" + host.replace(/"/g, "") + ", " : ""
    obfs = obfs + host
...
function Fobfs(jsonl, Pcert0, PTls13) {
  ...
  if (jsonl.net == "ws" && jsonl.tls == "tls") {
    ...
    host0 = jsonl.host && jsonl.host != "" ? "obfs-host=" + jsonl.host.replace(/"/g, "") + ", " : "";
...
function SSR2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.split(":").length <= 8) { //排除难搞的 ipv6 节点
    ...
    if (cnt.indexOf("obfsparam=") != -1 && obfs != "") {
      obfshost = cnt.split("obfsparam=")[1].split("&")[0] != "" ? "obfs-host=" + Base64.decode(cnt.split("obfsparam=")[1].split("&")[0].replace(/-/g, "+").replace(/_/g, "/")).split(",")[0].split("\u0000")[0].replace(/"/g, "") : ""
    }
...
function VL2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if ((cnt.indexOf("remarks=") == -1 && cnt.indexOf("remark=") == -1) && cnt.indexOf("@") != -1) { // normal URI
    ...
    thost = cnt.indexOf("sni=") != -1 ? "tls-host=" + cnt.split("sni=")[1].split(/&|#/)[0].replace(/"/g, "") : ""
    thost = cnt.indexOf("peer=") != -1 ? "tls-host=" + cnt.split("peer=")[1].split(/&|#/)[0].replace(/"/g, "") : thost
  ...
  if (typeU == "SR-URI") {//小火箭内的websocket写法
    ...
    thost = cnt.indexOf("&host=") == -1 ? thost : "obfs-host=" + decodeURIComponent(cnt.split("&host=")[1].split("&")[0].split("#")[0]).replace(/"/g, "")
    thost = cnt.indexOf("sni=") == -1 ? thost : "obfs-host=" + decodeURIComponent(cnt.split("sni=")[1].split("&")[0].split("#")[0]).replace(/\"|(Host\":)|\{|\}/g, "")
    puri = cnt.indexOf("&path=") == -1 ? puri : "obfs-uri=" + decodeURIComponent(cnt.split("&path=")[1].split("&")[0].split("#")[0])
  }
...
function TJ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (cnt.indexOf("&plugin=obfs-local") != -1) {//小火箭内的websocket写法
    ...
    thost = cnt.indexOf("obfs-host=") == -1 ? thost : "obfs-host=" + decodeURIComponent(cnt.split("obfs-host=")[1].split(";")[0].split("#")[0]).replace(/"/g, "")
...
function SS2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.split(":").length <= 10) { //排除难搞的 ipv6 节点
    ...
    if (cntt.indexOf("v2ray-plugin") == -1 && cntt.indexOf("plugin=v2ray") == -1) { //Shadowrocket style v2-plugin
      ...
      obfshost = cnt.split("obfs-host%3D")[1] != null ? ", obfs-host=" + cnt.split("obfs-host%3D")[1].split("&")[0].split("#")[0].replace(/"/g, "") : "";
    } else if (cnt1 != undefined) {
      cnt1 = JSON.parse(cnt1)
      obfs = cnt1.tls ? ", obfs=wss" : ", obfs=ws"
      obfshost = cnt1.host ? ", obfs-host=" + cnt1.host.replace(/"/g, "") + ", tls-verification=false" : ""
      ...
      obfshost = cnt1.indexOf("host=") != -1 ? ", obfs-host=" + cnt1.split("host=")[1].split(";")[0].split("#")[0].replace(/"/g, "").trim() : ""
...
function SSD2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.plugin_options) {
    ...
    obfshost = cnt.plugin_options.split(";")[1] != null ? ", " + cnt.plugin_options.split(";")[1].replace(/"/g, "") : "";
  }
  pudp = Pudp == 1 ? "udp-relay=true" : "udp-relay=false";
  ptfo = Ptfo == 1 ? "fast-open=true" : "fast-open=false";
  for (var i in cnt.servers) {
    ...
    if (cnt.servers[i].plugin_options) {
      ...
      obfshost = cnt.servers[i].plugin_options.split(";")[1] != null ? ", " + cnt.servers[i].plugin_options.split(";")[1].replace(/"/g, "") : "";
    }
...
function SCT2QX(content) {
  ...
  var phost = cnt.indexOf("obfs-host") != -1 ? "obfs-host" + cnt.split("obfs-host")[1].split(",")[0].replace(/"/g, "").trim() : "";
  if (phost != "") {
    pobfs = pobfs + ", " + phost
  }
...
function SVmess2QX(content) {
  ...
  var phost = cnt.indexOf("ws-headers") != -1 ? "obfs-host=" + cnt.split("ws-headers")[1].split(",")[0].split("=")[1].split(":")[1].replace(/"/g, "").trim() : "";
...
function Strojan2QX(content) {
  ...
  var ptfo = paraCheck(cnt, "tfo") == "true" ? "fast-open=true" : "fast-open=false";
  var pverify = cnt.replace(/ /g, "").indexOf("skip-cert-verify=false") != -1 ? "tls-verification=true" : "tls-verification=false";
  var phost = cnt.indexOf("sni") != -1 ? "tls-host=" + cnt.split("sni")[1].split(",")[0].split("=")[1].replace(/"/g, "") : ""
  pvefify = Pcert0 == 1 ? "tls-verification=true" : pverify;
  var ptls13 = paraCheck(cnt, "tls13") == "true" ? "tls13=true" : "tls13=false";
...
function LoonSS2QX(cnt) {
  ...
  var obfs = cnt.split(",").length == 7 ? ", " + ["obfs=" + cnt.split(",")[5].trim(), "obfs-host=" + cnt.split(",")[6].trim().replace(/"/g, "")].join(",") : ""
  var tag = ", tag=" + cnt.split("=")[0].trim()
...
function LoonVL2QX(cnt) {
  ...
  if (cnt.indexOf("host=") != -1) {
    obfshost = "obfs-host=" + cnt.split("host=")[1].split(",")[0].replace(/"/g, "")
  } else if (cnt.indexOf("tls-name=") != -1) {
    obfshost = "obfs-host=" + cnt.split("tls-name=")[1].split(",")[0].replace(/"/g, "")
  } else if (cnt.indexOf("sni=") != -1) {
    obfshost = "obfs-host=" + cnt.split("sni=")[1].split(",")[0].replace(/"/g, "")
  }
...
function CSS2QX(cnt) {
  ...
  ohost = cnt["plugin-opts"] ? "obfs-host=" + cnt["plugin-opts"].host.replace(/"/g, "") : ""
  ouri = ""
  cert = ""
  if (obfs.indexOf("websocket") != -1) {
    obfs = cnt["plugin-opts"].tls ? "obfs=wss" : "obfs=ws"
    ohost = cnt["plugin-opts"].host ? "obfs-host=" + cnt["plugin-opts"].host.replace(/"/g, "") : ""
...
function CSSR2QX(cnt) {
  ...
  obfs = "obfs=" + cnt.obfs
  if (cnt["obfs-param"]) {
    ohost = "obfs-host=" + cnt["obfs-param"].replace(/"/g, "")
  } else if (cnt["obfsparam"]) {
    ohost = "obfs-host=" + cnt["obfsparam"].replace(/"/g, "")
  }
...
function CV2QX(cnt) {
  ...
  ohost = cnt["ws-headers"] ? "obfs-host=" + cnt["ws-headers"]["Host"].replace(/"/g, "") : ""
  ohost = phost ? "obfs-host=" + phost.replace(/"/g, "") : ohost
  //ohost= cnt["ws-opts"]? "obfs-host=" + cnt["ws-opts"]["headers"]["Host"] : ohost
  ohost = cnt["servername"] ? "obfs-host=" + cnt["servername"].replace(/"/g, "") : ohost
  console.log(ohost)
...
function CT2QX(cnt) {
  ...
  if (cnt.network == "ws") { //wss类型
    ...
    if (cnt["ws-opts"]) {
      ...
      ohost = cnt["ws-opts"]["headers"] ? "obfs-host=" + cnt["ws-opts"]["headers"]["Host"].replace(/"/g, "") : ""
    }
